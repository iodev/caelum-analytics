...
class ClusterNode:
    ...
    async def send_message_to_machine(self, machine_id: str, message: ClusterMessage) -> bool:
    ...
    async def _handle_machine_discover(self, message: ClusterMessage, websocket):
        ...
        response = ClusterMessage(
            ...
            payload={
                'machine_info': machine_registry.machines[self.machine_id].to_dict(),
                'discovered_machines': [m.to_dict() for m in self.discovered_machines.values()]
            }
        )
        await websocket.send(response.to_json())
                ...
                response = ClusterMessage(
                    ...
                    payload={
                        ...
                        'estimated_completion': (datetime.now(timezone.utc) + 
                                               timedelta(seconds=task.estimated_duration)).isoformat()
                    }
                )
                await websocket.send(response.to_json())
    ...
    async def _can_provide_resources(self, resource_req: Dict, machine: MachineNode) -> bool:
        ...
        available_cpu = machine.resources.cpu_cores * (1 - machine.resources.cpu_usage_percent / 100)
        available_memory = machine.resources.memory_available_gb
        available_gpu = len(machine.resources.gpu_info) if machine.resources.gpu_info else 0
