...
class DistributedCodeAnalyzer:
    ...
    async def _process_chunk_locally(self, chunk: CodeChunk, task: AnalysisTask):
        ...
        try:
            ...
            results = await self._analyze_code_chunk(chunk, task.analysis_type, task.configuration)
    ...
    async def _analyze_code_chunk(self, chunk: CodeChunk, analysis_type: AnalysisType, configuration: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if analysis_type == AnalysisType.STATIC_ANALYSIS:
            ...
            avg_complexity = total_complexity / max(1, total_functions) if total_functions > 0 else 0
            ...
            results.update({
                ...
                'external_dependencies': len([i for i in results.get('all_imports', []) if not i.startswith('.')]),
            ...
            results.update({
                ...
                'documentation_coverage': self._calculate_doc_coverage(results['files_details']),
                'style_violations': len(warnings)
            })
        
        return results
    
    async def _analyze_single_file(self, file_path: str, analysis_type: AnalysisType) -> Dict[str, Any]:
        ...
        try:
            ...
            if file_path.suffix == '.py':
                results.update(await self._analyze_python_file(file_path, content, analysis_type))
            elif file_path.suffix in ['.js', '.ts']:
                results.update(await self._analyze_javascript_file(file_path, content, analysis_type))
            elif file_path.suffix in ['.java']:
                results.update(await self._analyze_java_file(file_path, content, analysis_type))
            else:
                results.update(await self._analyze_generic_file(file_path, content, analysis_type))
    ...
    async def _analyze_python_file(self, file_path: Path, content: str, analysis_type: AnalysisType) -> Dict[str, Any]:
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    ...
                    results['cyclomatic_complexity'] += self._calculate_cyclomatic_complexity_python(node)
            ...
            if analysis_type == AnalysisType.SECURITY_SCAN:
                results['security_issues'] = self._find_python_security_issues(tree, content)
            
            # Code quality checks
            if analysis_type in [AnalysisType.STATIC_ANALYSIS, AnalysisType.CODE_QUALITY]:
    ...
    async def _analyze_javascript_file(self, file_path: Path, content: str, analysis_type: AnalysisType) -> Dict[str, Any]:
    ...
    async def _analyze_java_file(self, file_path: Path, content: str, analysis_type: AnalysisType) -> Dict[str, Any]:
        """Analyze Java file."""
        import re
    ...
    async def _analyze_generic_file(self, file_path: Path, content: str, analysis_type: AnalysisType) -> Dict[str, Any]:
    ...
    def _find_python_security_issues(self, tree: ast.AST, content: str) -> List[Dict[str, Any]]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                ...
                if hasattr(node, 'end_lineno') and node.end_lineno:
                    ...
                    if func_length > 50:
                        warnings.append(f"Long function '{node.name}' ({func_length} lines)")
    ...
    async def _reassign_failed_chunk(self, chunk: CodeChunk, task: AnalysisTask, failed_machine: str):
        ...
        if not available_machines:
            logger.warning(f"No alternative machines available for chunk {chunk.chunk_id[:8]}, processing locally")
            await self._process_chunk_locally(chunk, task)
            return
        ...
        for machine in available_machines:
            ...
            session = self.active_sessions.get(task.task_id)
            if session:
                machine_loads[machine] = len([m for m in session.chunk_assignments.values() if m == machine])
            else:
        ...
        logger.info(f"Reassigning chunk {chunk.chunk_id[:8]} from {failed_machine} to {target_machine}")
